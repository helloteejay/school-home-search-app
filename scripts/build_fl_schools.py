...
Vintage note (Hillsborough): ratings are FL DOE 2023-24; the HCPS zone
polygons are the district's CURRENT 2025-26 locator layer (HCPS rezoned
districtwide effective 2024-25, and its public ArcGIS orgs only carry
stale 2020-21 layers). Ratings-year vs zone-year mismatch is inherent
until DOE publishes newer grades; the join is by school, not geography,
so a school keeps its grade even if its zone shifted shape.
...
# DOE "District Name" -> (app district key, boundary GeoJSON path).
# Adding a metro = one line here + one ENDPOINTS entry in
# fetch_school_boundaries.py.
DISTRICTS = {
    "BROWARD": ("broward", REPO / "data" / "boundaries" / "broward_elementary.geojson"),
    "MIAMI-DADE": ("miamidade", REPO / "data" / "boundaries" / "miamidade_elementary.geojson"),
    "HILLSBOROUGH": ("hillsborough", REPO / "data" / "boundaries" / "hillsborough_elementary.geojson"),
}
...
def main() -> int:
    ...
    sub = df[df["District Name"].isin(list(DISTRICTS))].copy()
    ...
    indexes = {}
    for doe_district, (key, gj_path) in DISTRICTS.items():
        indexes[doe_district] = load_boundary_index(gj_path)
        print(f"  {doe_district} index: {len(indexes[doe_district])} schools")
    ...
    for _, row in sub.iterrows():
        ...
        result = match_boundary(doe_name, district, indexes)
        if result is None:
    ...
    with OUT_PATH.open("w") as f:
        ...
        f.write(f"Source: FL DOE 2023-24 School Grades x district GIS "
