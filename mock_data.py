...
# ---------------------------------------------------------------------------
# School fixture data — A-rated boundary elementaries in Broward + Miami-Dade.
#
# Source: generated by scripts/build_fl_schools.py, which joins the FL DOE
# 2023-24 School Grades file with the district elementary-boundary GIS feeds.
# See data/schools_generated.py. The hand-coded fallback below is used only
# when the generated file doesn't exist (e.g. a fresh clone before running
# the build script). The generated list supersedes any overlap.
#
# Each dict carries:
...
try:
    from data.schools_generated import GENERATED_SCHOOLS as _AUTO_SCHOOLS  # type: ignore
except ImportError:
    _AUTO_SCHOOLS = None


_HANDCODED_BASELINE: List[Dict[str, Any]] = [
    # ---- Broward County ----
    # Weston cluster
...
# Use the generated list when available — it's 142 A-rated boundary
# elementaries across both counties (vs. 27 in the hand-coded baseline).
FL_SCHOOLS: List[Dict[str, Any]] = _AUTO_SCHOOLS or _HANDCODED_BASELINE
...
def _real_boundary_for(school: Dict[str, Any]) -> Optional[Polygon]:
    """Return the real attendance polygon for ``school`` if available.

    Two lookup paths:
    1. If the school dict carries ``gis_name`` + ``district`` (generated by
       scripts/build_fl_schools.py), use them directly. This is the fast
       path for the 142 auto-generated schools.
    2. Otherwise fall back to ``_BOUNDARY_MAP`` keyed by the school's
       display name. This handles the hand-coded baseline.
    """
    cache = _load_boundary_cache()

    # Path 1: generated dicts carry the GIS name + district straight from
    # the build script, so we can look up directly.
    gis_name = school.get("gis_name")
    district = school.get("district")
    if gis_name and district:
        for zip_part in (str(school.get("zip_code") or ""), ""):
            poly = cache.get(f"{district}::{gis_name}::{zip_part}")
            if poly is not None:
                return poly

    # Path 2: legacy hand-coded path via _BOUNDARY_MAP.
    mapping = _BOUNDARY_MAP.get(school.get("name", ""))
    if not mapping:
        return None
